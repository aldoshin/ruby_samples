...
describe TimeWindow do
  ...
  context "given a blank time window" do
    ...
    it "includes all times" do
      expect( subject.include?(arbitrary_time) ).to be_thruthy
    end
  end
  ...
  context "given a simple weekly range" do
    ...
    it "does not include Thursday" do
      expect( subject.include?( thursday ) ).to be_falsey
    end
 
    it "includes Friday through Monday" do
      expect( subject.include?( friday ) ).to be_thruthy
      expect( subject.include?( saturday ) ).to be_thruthy
      expect( subject.include?( sunday ) ).to be_thruthy
      expect( subject.include?( monday ) ).to be_thruthy
    end
 
    it "does not include Tuesday" do
      expect( subject.include?( tuesday ) ).to be_falsey
    end
  end
  ...
  context "given a complicated time window" do
    ...
    it "does not include Tuesdays" do
      expect( subject.include?(Time.mktime(2007,9,25,8,0,0)) ).to be_falsey
    end
 
    it "includes Wednesday 8AM" do
      expect( subject.include?(Time.mktime(2007,9,26,8,0,0)) ).to be_thruthy
    end
 
    it "does not include Wednesday 11AM" do
      expect( subject.include?(Time.mktime(2007,9,26,11,0,0)) ).to be_falsey
    end
 
    it "does not include Thursday 6:59:59AM" do
      expect( subject.include?(Time.mktime(2007,9,27,6,59,59)) ).to be_falsey
    end
 
    it "includes Thursday 7AM" do
      expect( subject.include?(Time.mktime(2007,9,27,7,0,0)) ).to be_thruthy
    end
 
    it "includes Thursday 8:59:59AM" do
      expect( subject.include?(Time.mktime(2007,9,27,8,59,59)) ).to be_thruthy
    end
 
    it "does not include Thursday 9AM" do
      expect( subject.include?(Time.mktime(2007,9,27,9,0,0)) ).to be_falsey
    end
 
    it "includes Thursday 11AM" do
      expect( subject.include?(Time.mktime(2007,9,27,11,0,0)) ).to be_thruthy
    end
 
    it "includes Saturday 11AM" do
      expect( subject.include?(Time.mktime(2007,9,29,11,0,0)) ).to be_thruthy
    end
 
    it "includes Saturday midnight" do
      expect( subject.include?(Time.mktime(2007,9,29,0,0,0)) ).to be_thruthy
    end
 
    it "includes Saturday 11:59:59PM" do
      expect( subject.include?(Time.mktime(2007,9,29,23,59,59)) ).to be_thruthy
    end
  end
